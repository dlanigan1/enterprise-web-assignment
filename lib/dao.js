...
var borrowedBooks = data.borrowedBooks;
...
dao.loadBorrowedBooks = function() {
  Schemas.borrowedBookModel.find({}).remove(() => {
    Schemas.borrowedBookModel.collection.insert(borrowedBooks, (err, docs)=>{
      if (err) {
        console.log(`failed to Load borrowed Book Data: ${err}`);
      } else {
        console.info(`${borrowedBooks.length} borrowed Book levels were successfully stored.`);
      }
    });
  });
};
...
/**
 * Get a list of the library book titles
 *
 * returns Book title
 **/
dao.getAllBookTitles = function() {
  return Schemas.bookModel.find({},{ "title": 1,"_id": 0 });
}
...
dao.getAllGenreTypes = function() {
  return Schemas.genreTypeModel.find({}).select('genretype -_id');
}
/**
...
dao.getAllStatusTypes = function() {
  return Schemas.statusTypeModel.find({}).select('statustype -_id');
}
...
dao.updateBook = function(id,body) {
  ...
  .then(function (data) {
      console.info(`${body.title} was successfully updated.`);
      return Promise.resolve(data);
  });
